...
from sqlalchemy import create_engine                                              # Importamos 'create_engine' de la libreriá 'sqlalchemy', hay que instalarlo en consolo/virtual env usando "conda install sqlalchemy" permite tener acceso a las funcionalidades de orm (object relational mapper: nos permite trabajar con objetos de python en lugar de querys de SQL directamente) de sqlalchemy
...
engine = create_engine('sqlite:///newspapers.db')                                 # Acá le digo a 'sqlalchemy' que quiero utlizar 'sqllite', ya viene instalado dentro de la distribución de Python, y le paso como parámetro el nombre que tendra el archivo de la base de datos, le decimos a sqlalchemy que queremos usar sqlite
