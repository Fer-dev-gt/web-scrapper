...
def _save_articles(news_site_uid, articles):                                                        # Guarda los archivos del Scrapper al crear un archivo CSV, recibe el "News_site_uid" y la lista de objetos de articulos encontrados validos 
  ...
  with open(out_file_name, mode='w+') as file:                                                      # Abrimos el archivo "out_file_name" con el modo "w+" que instruye que es para escribir y si no existe el archivo lo tiene que crear y definimos la referencia del archivo como "file"
...
if __name__ == '__main__':                                                                          # Entry Point
  parser = argparse.ArgumentParser()                                                                # Instanciamos un objeto de la clase ArgParser para darle parametros al archivo y ejecutarlos con el nombre del periodico para extraer las noticias
